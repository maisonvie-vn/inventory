  ...
  const refreshSupabaseData = async () => {
    ...
    try {
      ...
      const viewName = currentUser?.role === 'admin' ? 'v_inventory_finance' : 
                       (currentUser?.role === 'restaurant_manager' || currentUser?.role === 'junior_accountant' || currentUser?.role === 'head_chef') ? 'v_inventory_cost' : 'v_inventory_ops';
  ...
  const handleSaveManualGrn = async (e: React.FormEvent) => {
    ...
    if (isSupabaseConfigured()) {
      try {
        ...
        const rawUserId = user?.id || currentUser?.id || null;
