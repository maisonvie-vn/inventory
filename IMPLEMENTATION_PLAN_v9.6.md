# KẾ HOẠCH TRIỂN KHAI HỆ THỐNG CRM/ERP INVENTORY – MAISON VIE **v9.6**
### (NỀN MÓNG ĐÃ TRIỂN KHAI 17/06 · §14 UNMAPPED + NHẬP TAY · §16 deduction_type · §17 NÂNG CẤP: UUID+CODE SỬA ĐƯỢC · NON-SALE · PO DUYỆT · MIN-STOCK MÙA VỤ · SETTINGS 3-VÙNG)
...
---
*(Chốt trạng thái 17/06: nền móng RLS/UoM/secret/dark theme/mobile + phần lớn nghiệp vụ đã triển khai. Nâng cấp v9.6 ở §17 bên dưới.)*
...
# 17. NÂNG CẤP v9.6 — MASTER-DATA & QUẢN TRỊ (6 quyết định đã chốt 17/06)

## 17.1. `ingredients.id` → UUID + cột `code` SỬA ĐƯỢC *(đảo §11.1)*
**Lý do:** yêu cầu "đổi mã → cả hệ tự cập nhật" không thể có nếu code là PK bất biến. Chuyển PK sang uuid; `code` thành cột thường, **unique, sửa được, có audit**. FK trỏ uuid ⇒ đổi `code` chỉ là sửa một cột, **không cascade, không vỡ**.

**Kế hoạch migration (an toàn, theo bước):**
1. Thêm `id uuid default gen_random_uuid()` vào `ingredients` (+ unique); giữ `code` hiện có.
2. Thêm cột `ingredient_uuid` vào MỌI bảng con (recipes, lots, inventory_transactions, grn_lines, pos_alias_map, ...); **backfill** bằng join theo `code`.
3. Tạo FK mới trỏ `ingredient_uuid → ingredients.id`; kiểm đối chiếu 100% (không NULL, không mồ côi).
4. Hạ `code` xuống cột thường **unique** (giữ validate định dạng §11.1: VIẾT HOA, không dấu cách); cho phép sửa, **mỗi lần sửa ghi audit (cũ→mới, ai, khi nào)**.
5. Sau khi xác minh, gỡ FK/PK cũ theo `code`.
> Lưu ý: `code` vẫn là mã hiển thị/in/quét — đổi code **không** đụng tham chiếu nội bộ (đã trỏ uuid), nhưng phải **đồng bộ lại nhãn in/POS alias ngoài** nếu có.

## 17.2. Cơm NV / hướng dẫn / tasting = `non_sale_consumption` (KHÔNG phải sale giá 0)
```sql
create table if not exists non_sale_consumption (
  id uuid primary key default gen_random_uuid(),
  business_date date not null,
...
```
- Sinh `inventory_transactions` **OUT**, `source='NON_SALE'`. **Trừ kho** nhưng **KHÔNG vào doanh thu** ⇒ food cost % không bị méo.
- **Vẫn vào tổng tiêu hao** để variance đúng; báo cáo có dòng "chi phí ngoài bán hàng" riêng.
- Điểm vào "hàng bán không giá" trên UI **định tuyến về đây**, không vào luồng Sales.

## 17.3. PO THỦ CÔNG có DUYỆT + hiển thị days-of-cover
**Trạng thái PO:** `DRAFT → PENDING_APPROVAL → APPROVED → SENT → PARTIAL → RECEIVED → CLOSED`.
**Form PO thủ công — mỗi dòng bắt buộc hiện:** tồn hiện tại · tiêu hao TB/ngày · **days-of-cover** · SL gợi ý (auto) · **SL đang trên đường (net open PO)**.
**Cổng duyệt:** nếu `tổng giá trị > NGƯỠNG` (một **setting tài chính, effective-dated** §17.5) ⇒ phải `APPROVED` bởi Manager/CFO trước khi `SENT`; đặt SL cao bất thường ⇒ **bắt nhập lý do**. Mọi bước **audit**.
**Chống đặt trùng:** trừ net open PO để không đặt lại hàng đang về.

## 17.4. MIN-STOCK: tính tự động + override tay + hồ sơ mùa
- **Điểm đặt hàng (reorder point) TÍNH RA:** `tiêu hao_TB_ngày × lead_time_ngày × hệ_số_an_toàn`, một job định kỳ tính lại từ tiêu hao cuộn (rolling).
- **Override tay/mặt hàng:** `min_override` + lý do, **audit** (ngoại lệ, không phải cách chính).
- **Hồ sơ mùa:** `seasonal_profile(name, multiplier, date_range)`; **min hiệu lực = (override nếu có) else (derived × hệ_số_mùa đang active)**. Đặt nền một lần, đổi mùa thì mọi min tự scale — **không gõ tay từng mặt hàng mỗi mùa**.

## 17.5. MODULE SETTINGS — 3 vùng, effective-dated, chỉ Admin/Owner
**Phân vùng theo bán kính sát thương:**
| Vùng | Ví dụ | Quy tắc |
| :-- | :-- | :-- |
| **Vô hại** | tên NH, logo, email-from | sửa thoải mái + audit |
| **Vận hành** | giờ cron, người nhận cảnh báo, mặc định reorder | sửa + audit, hiệu lực từ nay |
| **⛔ Tài chính** | VAT, phí phục vụ, giá gồm/chưa-VAT, **buffer 10%**, **ngưỡng duyệt PO**, làm tròn, tiền tệ | **effective-dated (versioned)** + audit + lý do |
...
-- Vùng FINANCIAL: KHÔNG ghi đè — lưu phiên bản theo ngày hiệu lực
create table if not exists app_settings_history (
  ...
  changed_by uuid, reason text, created_at timestamptz default now(),
  primary key (key, valid_from)
);
```
**Luật bắt buộc:**
- Đọc setting tài chính theo **ngày giao dịch** (lấy phiên bản hiệu lực tại ngày đó) ⇒ **quá khứ không bị viết lại**.
- **Secret KHÔNG ở đây** → Vault; UI chỉ hiện "đã cấu hình ✓" + nút xoay khóa (ghi server-side).
- **Guard-rails:** không xóa admin cuối, chặn giá trị phá hệ (VAT 200%, buffer âm, cron sai), mặc định an toàn.
- **Truy cập:** CHỈ Admin/Owner sửa **và** module chỉ render trong khu Admin/Owner (vai trò khác không thấy).
- **Toàn vẹn không phụ thuộc truy cập:** hạn chế người sửa KHÔNG bỏ effective-dating/audit — kể cả Owner đổi cũng để lại vết và không rewrite quá khứ.
- **Setting bắt buộc:** `timezone='Asia/Ho_Chi_Minh'` + **giờ chốt ngày kinh doanh** (vd 04:00) — đồng bộ với cron (UTC §13) và chốt sổ §3.
- **Tách settings (chính sách) khỏi master-data (danh sách thực thể: NCC, nguyên liệu, user, UoM).**

---
*(Nâng cấp v9.6 §17 ở trên. §18 = đề xuất bổ sung; mục CAO 1–5 đã chốt, chi tiết §19.)*

---

# 18. ĐỀ XUẤT BỔ SUNG CHO HỆ INVENTORY *(mục 1–5 ĐÃ CHỐT 17/06 → chi tiết §19; mục 6–8 chờ duyệt)*

> Theo yêu cầu "nếu thấy cần thêm thì báo lại". Đây là các khoảng trống tôi đánh giá là **thật sự quan trọng** với một hệ inventory nhà hàng fine-dining chuyên nghiệp, chưa nằm trong plan. Xếp ưu tiên để bạn chọn.

| # | Hạng mục | Vì sao cần | Ưu tiên |
| :-- | :-- | :-- | :--: |
| 1 | **Lô/Hạn dùng/FEFO + truy vết** | Đồ tươi + an toàn thực phẩm. Trừ kho theo lô hết hạn trước; truy được "lô NCC nào vào món nào ngày nào" khi cần thu hồi/HACCP. Hiện có cron cảnh báo hạn nhưng cần **mô hình lô thật**. | **CAO** |
| 2 | **Hệ số hao hụt sơ chế (yield/trim)** | Cá nguyên con, rau củ... gọt/lọc mất 20-40%. Giá thành phải tính trên **phần dùng được**, không phải SL mua — nếu không **food cost bị tính thấp giả tạo**. | **CAO** |
| 3 | **Công thức con / mẻ sản xuất (mise en place / BTP)** | Bếp nấu sốt nền, demi-glace, bột... theo mẻ rồi dùng dần. Cần **sub-recipe + sản lượng mẻ**: nguyên liệu → BTP (nhập kho BTP) → món. Fine-dining rất nhiều khâu này. | **CAO** |
| 4 | **Bảng giá NCC + đa NCC/mặt hàng + cảnh báo biến động giá mua (PPV)** | PO tự điền giá; so giá nhiều NCC; **báo động khi giá nhập tăng** (ảnh hưởng giá vốn & giá menu). Kiểm soát chi phí thật sự. | **CAO** |
| 5 | **Kiểm kê định kỳ / cycle-count có duyệt variance** | Ngoài chốt ngày: đếm toàn phần định kỳ (phiếu đếm, freeze-count, **duyệt chênh lệch** rồi mới post điều chỉnh); đếm hàng giá trị cao thường xuyên hơn. Đây là "input đếm thực tế" đã nhắc. | **CAO** |
| 6 | **Báo cáo định giá tồn kho (stock value × WAC theo thời gian)** | Con số **tài sản tồn kho** — nối thẳng sang tài chính/bảng cân đối; theo dõi dead-stock, days-of-inventory. | TRUNG |
| 7 | **Menu engineering (margin/món + sales-mix)** | Biến dữ liệu công thức + bán hàng thành **quyết định giá & menu** (ma trận sao/ngựa-kéo/chó). Có dữ liệu rồi, chỉ cần lớp phân tích. | TRUNG |
| 8 | **Preview-trước-khi-commit cho import + chính sách tồn âm** | Mọi import (POS/GRN/danh mục) cần **xem trước + validate** rồi mới ghi, tránh ghi sai hàng loạt. Định rõ **tồn âm**: cho-kèm-cờ hay chặn. | TRUNG |

**Khuyến nghị thứ tự:** làm xong phần đang dở (§14 unmapped + nhập tay, §17.1 uuid, §17.5 settings) → rồi nhóm **CAO** (1→5) vì chúng ảnh hưởng trực tiếp độ chính xác giá vốn & an toàn thực phẩm → cuối cùng nhóm TRUNG (6→8) là lớp phân tích/kiện toàn.

---

# 19. CHI TIẾT 5 HẠNG MỤC CAO ĐÃ CHỐT (17/06) — schema + quy trình

> Năm mục này khớp nối nhau: **lô/FEFO (19.1)** áp cho cả nguyên liệu thô và BTP; **yield (19.2)** + **công thức con (19.3)** quyết định giá vốn thật mỗi món; **giá NCC/PPV (19.4)** nuôi WAC; **kiểm kê (19.5)** chốt tồn thực tế. Giá vốn vẫn là **Moving WAC ở cấp SKU**; lô dùng cho FEFO/hạn/truy vết, KHÔNG thay WAC bằng FIFO.

## 19.1. Lô / Hạn dùng / FEFO + Truy vết
```sql
create table if not exists lots (
  id uuid primary key default gen_random_uuid(),
  ingredient_id uuid not null references ingredients(id),
  lot_code text, supplier_id uuid, grn_id uuid,
  received_date date, expiry_date date,
  qty_received numeric not null, qty_remaining numeric not null,
  unit_cost numeric,                      -- để truy vết; định giá báo cáo vẫn WAC
  status text default 'ACTIVE' check (status in ('ACTIVE','EXPIRED','DEPLETED'))
);
-- giao dịch trừ kho gắn lot_id để truy vết
alter table inventory_transactions add column if not exists lot_id uuid references lots(id);
```
**Quy tắc:** trừ kho theo **FEFO** (hạn gần nhất trước) → giảm `qty_remaining`. Lô quá hạn → cảnh báo (cron §13) + **chặn/cờ** khi định xuất. **Truy vết HACCP:** `lot → inventory_transactions → sale_line/ngày/món` (thu hồi khi cần). WAC để định giá; lô để FEFO + hạn + truy vết (không trộn hai khái niệm).

## 19.2. Hệ số hao hụt sơ chế (Yield / Trim)
```sql
...
-- (tùy chọn) override theo dòng công thức
alter table recipe_lines add column if not exists yield_pct_override numeric;
```
**Quy tắc:** định lượng công thức nhập theo **phần DÙNG ĐƯỢC**; hệ **quy ngược ra SL MUA** để trừ kho + tính giá: `SL_mua = SL_dùng / yield%`. **Giá vốn thật = WAC / yield%** (cá nguyên con 1kg, yield 60% → giá usable cao hơn 1,67×). **Phần hao trim là hao hụt ĐƯỢC TÍNH TRƯỚC, KHÔNG bị gắn cờ variance/thất thoát** — đây là điểm chí mạng để food cost không bị tính thấp giả tạo.

## 19.3. Công thức con / Mẻ sản xuất (sub-recipe / BTP)
```sql
create table if not exists production_orders (
  id uuid primary key default gen_random_uuid(),
  output_ingredient_id uuid not null references ingredients(id),  -- BTP cũng là 1 SKU
  output_qty numeric not null,           -- sản lượng mẻ (yield thực)
...
create table if not exists production_inputs (
  ...
  ingredient_id uuid references ingredients(id), qty numeric not null
);
```
**Quy tắc:** nấu một mẻ → **inputs OUT**, **BTP IN** với giá `tổng chi phí input / output_qty` (nuôi WAC của BTP). BTP là **SKU bình thường** → có thể là nguyên liệu trong công thức món khác ⇒ **BOM nhiều tầng** (món → demi-glace → xương/rau), giá vốn **roll-up qua tầng**. BTP **có lô + hạn** (§19.1, FEFO). Sản lượng thực vs lý thuyết = **variance sản xuất**.

## 19.4. Bảng giá NCC + Đa NCC + Cảnh báo biến động giá mua (PPV)
```sql
create table if not exists suppliers (
  id uuid primary key default gen_random_uuid(),
  name text not null, contact text, payment_terms text,
  lead_time_days int, is_active boolean default true     -- soft-delete
);
...
```
**Quy tắc:** một nguyên liệu có **nhiều NCC** + một NCC **ưu tiên**; PO **tự điền giá** từ bảng giá. Khi GRN: so giá nhận thực tế vs giá tham chiếu (lần trước/chuẩn) → **PPV**; vượt ngưỡng (setting §17.5) → **cảnh báo "giá X tăng Y%"** + gợi ý rà lại giá menu. Giá nhập mới nuôi WAC như thường.

## 19.5. Kiểm kê định kỳ / Cycle-count có DUYỆT variance
```sql
create table if not exists stocktakes (
  id uuid primary key default gen_random_uuid(),
  type text check (type in ('FULL','CYCLE')), scope text,  -- category/location
  business_date date,
  status text default 'DRAFT' check (status in
    ('DRAFT','COUNTING','REVIEW','APPROVED','POSTED')),
  started_by uuid, approved_by uuid
);
create table if not exists stocktake_lines (
  stocktake_id uuid references stocktakes(id),
  ingredient_id uuid references ingredients(id),
  system_qty numeric,     -- SNAPSHOT lúc freeze
  counted_qty numeric, variance numeric, variance_value numeric, reason text
);
```
**Quy tắc:** **Freeze** = chụp `system_qty` để giao dịch đang chạy không làm sai phiếu đếm → nhập `counted_qty` → **review variance** (chênh lớn bắt nhập lý do) → **APPROVED** (Manager/CFO cho hàng giá trị cao) → **POST** điều chỉnh (`inventory_transactions` ADJUST). **Cycle plan:** hàng giá trị cao đếm thường xuyên hơn (tuần), full count theo tháng. Điều chỉnh đã post = **audit + reversal, không sửa lịch sử**. Nối thẳng cột variance thô/buffer §5.

---
*Hết bản v9.6 (17/06/2026). Đã chốt: §17 (6 quyết định master-data) + §19 (5 hạng mục CAO: lô/FEFO, yield, công thức con, giá NCC/PPV, kiểm kê). Còn chờ duyệt: §18 mục 6–8 (định giá tồn WAC, menu engineering, preview-import + tồn âm). Trọng tâm build: §14 → §17.1 (uuid) → §17.5 (settings) → §19 nhóm CAO.*
